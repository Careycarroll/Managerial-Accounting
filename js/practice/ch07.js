// js/practice/ch07.js
// Wires the Ch. 7 practice page:
//  - Renders picker tiles from ch07Problems
...
function renderPicker() {
  pickerGrid.innerHTML = ch07Problems.map((problem) => {
    const minutes   = problem.estimatedMinutes ?? 6;
  ...
  pickerGrid.querySelectorAll('.practice-picker-tile').forEach((tile) => {
    tile.addEventListener('click', () => {
      ...
      const problem = ch07Problems.find((p) => p.id === id);
